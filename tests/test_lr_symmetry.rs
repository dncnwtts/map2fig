...
fn test_left_right_symmetry_detail() {
    ...
    for y in 63..139 {
        ...
        let left_extent_start = left_pixels.iter().filter(|&&x| x < cbar_left).min().copied();
        let left_extent_end = cbar_left.min(
            left_pixels.iter().filter(|&&x| x < cbar_left).max().copied().unwrap_or(cbar_left - 1)
        );
        ...
        let right_extent_start = (cbar_right + 1).max(
            left_pixels.iter().filter(|&&x| x > cbar_right).min().copied().unwrap_or(cbar_right + 1)
        );
        let right_extent_end = left_pixels.iter().filter(|&&x| x > cbar_right).max().copied().unwrap_or(cbar_right);
