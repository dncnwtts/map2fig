...
fn test_left_right_symmetry_detail() {
    ...
    for y in 63..139 {
        ...
        for x in cbar_left as u32..=cbar_right as u32 {
        ...
        let left_extent_start = left_pixels.iter().filter(|&&x| (x as i32) < cbar_left).min().copied();
        let left_extent_end = (cbar_left as u32).min(
            left_pixels.iter().filter(|&&x| (x as i32) < cbar_left).max().copied().unwrap_or(cbar_left as u32 - 1)
        );
        ...
        let right_extent_start = (cbar_right as u32 + 1).max(
            left_pixels.iter().filter(|&&x| (x as i32) > cbar_right).min().copied().unwrap_or(cbar_right as u32 + 1)
        );
        let right_extent_end = left_pixels.iter().filter(|&&x| (x as i32) > cbar_right).max().copied().unwrap_or(cbar_right as u32);
        
        let right_width = if (right_extent_start as i32) <= cbar_right {
            0
    ...
    if asymmetries.is_empty() {
        ...
        for (y, lw, rw) in asymmetries.iter().take(10) {
            println!("  y={}: LEFT extent width={}, RIGHT extent width={}", y, lw, rw);
        }
    }
}
