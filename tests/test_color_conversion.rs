...
mod color_conversion_tests {
    ...
    fn test_color_red() {
        ...
        let bgra = vec![red_rgba[2], red_rgba[1], red_rgba[0], red_rgba[3]];
    ...
    fn test_color_green() {
        ...
        let bgra = vec![green_rgba[2], green_rgba[1], green_rgba[0], green_rgba[3]];
        
        // In BGRA for Cairo: B=0, G=255, R=0, A=255
        assert_eq!(bgra, vec![0, 255, 0, 255]);
    }

    #[test]
    fn test_color_blue() {
        let blue_rgba = Rgba([0, 0, 255, 255]);
        let bgra = vec![blue_rgba[2], blue_rgba[1], blue_rgba[0], blue_rgba[3]];
        
        // In BGRA for Cairo: B=255, G=0, R=0, A=255
        assert_eq!(bgra, vec![255, 0, 0, 255]);
    }

    #[test]
    fn test_semi_transparent_color() {
        let semi_rgba = Rgba([100, 150, 200, 128]); // 50% transparent
        let bgra = vec![semi_rgba[2], semi_rgba[1], semi_rgba[0], semi_rgba[3]];
        
        // In BGRA for Cairo: B=200, G=150, R=100, A=128
        assert_eq!(bgra, vec![200, 150, 100, 128]);
    }
}
