...
fn test_colorbar_geometry() {
    ...
    if left_extent.1 > 0 {
        println!("LEFT extend at y=100: x={} to x={} (width={})", left_extent.0, left_extent.1, left_extent.1 - left_extent.0 + 1);
    }
    
    if right_extent.1 > 0 {
        println!("RIGHT extend at y=100: x={} to x={} (width={})", right_extent.0, right_extent.1, right_extent.1 - right_extent.0 + 1);
    }
}
