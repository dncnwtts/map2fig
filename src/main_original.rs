...
fn main() {
    ...
    if args.len() != 3 {
        eprintln!("Usage: {} <fits_file> <column_index (0-based)>", args[0]);
        std::process::exit(1);
    }

    let filename = &args[1];
    let col_index: usize = args[2]
        .parse()
        .expect("Column index must be a non-negative integer");
    ...
    // ----------------------------
    // Read binary table HDUs
    // ----------------------------
    while let Some(Ok(hdu)) = fits.next() {
        if let HDU::XBinaryTable(hdu) = hdu {
            println!("Reading binary table column {}...", col_index);

            let data = fits.get_data(&hdu);
            let mut table = data.table_data();

            let selected = table.select_fields(&[
                ColumnId::Index(col_index),
            ]);
            ...
            for cell in selected {
                  match cell {
                      ...
                      other => {
                          panic!("Unsupported column type: {:?}", other);
                  }
    ...
    // Summary
    // ----------------------------
    println!("Finished reading column {}", col_index);
    println!("Total values read: {}", values.len());
    println!(
        "First 10 values: {:?}",
        &values[..10.min(values.len())]
    );

    // Optional HEALPix sanity check
    // println!("Expected NPIX = {}", 12 * 512 * 512);
}
