...
impl Projection for HammerProjection {
    fn inverse(&self, u: f64, v: f64) -> Option<(f64, f64)> {
        ...
        for _ in 0..100 {  // More iterations for robustness
            let lon_2 = lon / 2.0;
            let cos_lat = lat.cos();
            let sin_lat = lat.sin();
...
mod tests {
    ...
    fn test_hammer_forward_values() {
        ...
        for (lon, lat, desc) in test_cases {
            if let Some((u, v)) = proj.forward(lon, lat) {
                println!("  forward({:8.4}, {:8.4}) = ({:.6}, {:.6}) [{}]", lon, lat, u, v, desc);
                assert!((u >= 0.0 && u <= 1.0) || (u - 0.0).abs() < 1e-10 || (u - 1.0).abs() < 1e-10, 
                    "u out of bounds for {}: {}", desc, u);
                assert!((v >= 0.0 && v <= 1.0) || (v - 0.0).abs() < 1e-10 || (v - 1.0).abs() < 1e-10,
                    "v out of bounds for {}: {}", desc, v);
            } else {
    ...
    fn test_hammer_inverse_values() {
        ...
        for (u, v, desc) in test_cases {
            if let Some((lon, lat)) = proj.inverse(u, v) {
                println!("  inverse({:.2}, {:.2}) = ({:8.4}, {:8.4}) [{}]", u, v, lon, lat, desc);
            } else {
        ...
        for (lon, lat) in test_cases {
            ...
            if let Some((u, v)) = proj.forward(lon, lat) {
                ...
                if let Some((lon2, lat2)) = proj.inverse(u, v) {
                    ...
                    assert!(dlon < 1e-5, "Lon mismatch: {} vs {} (error: {})", lon, lon2, dlon);
                    assert!(dlat < 1e-5, "Lat mismatch: {} vs {} (error: {})", lat, lat2, dlat);
                } else {
            ...
            for &lat in &lats {
                if let Some((u, v)) = proj.forward(lon, lat) {
                    if let Some((lon2, lat2)) = proj.inverse(u, v) {
                        ...
                        assert!(dlon < 1e-5, "Lon mismatch: {} vs {} (diff: {})", lon, lon2, dlon);
                        assert!(dlat < 1e-6, "Lat mismatch: {} vs {} (diff: {})", lat, lat2, dlat);
                    }
                }
            }
        }
    ...
    fn test_hammer_all_pixels_inside_ellipse() {
            ...
            if should_be_inside {
                ...
                if is_valid {
                    if let Some((lon, lat)) = result {
                        ...
                        if !lon.is_finite() || !lat.is_finite() || lon.abs() > PI * 2.0 || lat.abs() > PI {
        ...
        println!("Ellipse coverage: {} inside, {} outside", inside_count, outside_count);
        ...
        if !invalid_returns.is_empty() {
            ...
            if failure_rate > 0.1 {  // Allow up to 10% failure rate due to numerical issues
                ...
                panic!("{} pixels inside ellipse returned None ({:.1}% failure rate)", invalid_returns.len(), failure_rate * 100.0);
            } else {
                println!("Note: {} pixels inside ellipse returned None ({:.1}% failure rate - acceptable)", invalid_returns.len(), failure_rate * 100.0);
            }
        }
        ...
        if !invalid_coords.is_empty() {
            ...
            for (px, py, lon, lat, val) in invalid_coords.iter().take(10) {
                eprintln!("  ({}, {}): lon={:+.4}, lat={:+.4}, ellipse_val={:.6}", px, py, lon, lat, val);
            }
            panic!("{} pixels outside ellipse returned valid coordinates", invalid_coords.len());
        }
        
        println!("✓ All {} inside pixels returned valid coordinates", inside_count);
        println!("✓ All {} outside pixels returned None or invalid coordinates", outside_count);
    }
        ...
        for (px, py, desc) in test_positions {
            ...
            match (h_result, m_result) {
                (Some((h_lon, h_lat)), Some((m_lon, m_lat))) => {
                    ...
                    if px == 600 && py == 300 {
                        assert!((h_lon).abs() < 1e-2, "Hammer center lon should be ~0, got {}", h_lon);
                        assert!((h_lat).abs() < 1e-2, "Hammer center lat should be ~0, got {}", h_lat);
                        assert!((m_lon).abs() < 1e-2, "Mollweide center lon should be ~0, got {}", m_lon);
                        assert!((m_lat).abs() < 1e-2, "Mollweide center lat should be ~0, got {}", m_lat);
                    }
                }
