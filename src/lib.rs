...
/// Resolve the "bad" pixel color to an actual RGBA value
pub fn resolve_bad_color(bad: Option<BadColor>, cmap: &Colormap) -> Rgba<u8> {
    match bad.unwrap_or(BadColor::Auto) {
        BadColor::Auto => {
            // Use colormap underflow color as default
            let c = cmap.under();
...
#[cfg(test)]
mod tests {
    use super::*;  // everything in the library is now accessible
    ...
    fn test_bad_color_parse() {
        let auto = BadColor::from_str("auto").unwrap();
        match auto { BadColor::Auto => (), _ => panic!() }

        let gray = BadColor::from_str("gray").unwrap();
        match gray { BadColor::Gray => (), _ => panic!() }

        let rgba = BadColor::from_str("255,128,0,255").unwrap();
        match rgba { BadColor::Rgba(255,128,0,255) => (), _ => panic!() }
    }
}
...
fn test_resolve_bad_color() {
    ...
    let auto_color = resolve_bad_color(Some(BadColor::Auto), cmap);
    assert_eq!(auto_color.0[3], 255);

    let gray_color = resolve_bad_color(Some(BadColor::Gray), cmap);
    assert_eq!(gray_color, Rgba([128,128,128,255]));

    let custom_color = BadColor::Rgba(10,20,30,40);
    let c = resolve_bad_color(Some(custom_color), cmap);
