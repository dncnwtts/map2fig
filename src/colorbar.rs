...
pub fn apply_gamma(t: f64, gamma: f64) -> f64 {
    if gamma == 1.0 { t } else { t.powf(gamma) }
}
...
pub fn format_tick_label_with_units(
    value: f64,
    scale: Scale,
    pos: Option<f64>,
            ...
            if is_vertical_base {
                ...
                let round_half_to_even = |x: f64| {
                    ...
                    if frac < 0.5 {
                        ...
                        if as_int % 2 == 0 { as_int } else { as_int + 1 }
                    }
                };
...
fn edge_x_at_y(p1: (i32, i32), p2: (i32, i32), y: i32) -> Option<i32> {
    ...
    let t_num = if y2 > y1 { y - y1 } else { y1 - y };
...
mod tests {
    ...
    fn test_extend_triangles_symmetric() {
        ...
        assert!(
            (left_displacement - right_displacement).abs() < 0.01,
    ...
    fn test_extend_triangle_rounding_symmetry() {
        ...
        assert_eq!(
            left_max_pixel - left_min_pixel,
            right_max_pixel - right_min_pixel,
    ...
    fn test_extend_triangle_vertical_symmetry() {
        ...
        assert!(
            (top_slope_magnitude - bottom_slope_magnitude).abs() < 0.0001,
            "Slopes should have equal magnitude: top={:.4}, bottom={:.4}",
            top_slope_magnitude,
            bottom_slope_magnitude
        );
    ...
    fn test_edge_discrete_derivative_consistency() {
        ...
        for y in 621..=635 {
            ...
            eprintln!(
                "y={:3}: t={:6.4}, x_float={:7.3}, x_rounded={:3}, dx={}",
                y, t, x_float, x, dx
            );
    ...
    fn test_triangle_edge_no_plateaus() {
        ...
        eprintln!(
            "Top edge plateaus: {}, Bottom edge plateaus: {}",
            top_plateaus, bottom_plateaus
        );
        ...
        assert!(
            top_plateaus <= 1,
            "Top edge should have at most 1 plateau (at tip vertex)"
        );
    ...
    fn test_vectorized_edge_alignment() {
        ...
        assert!(
            (slope + 1.0).abs() < 0.01,
            "Top edge should have -1.0 slope for 45-degree diagonal, got {}",
            slope
        );
        ...
        for y in 621..=635 {
            ...
            if distance > 0.1 {
                eprintln!(
                    "  y={}: pixel_center=({:.1},{:.1}), expected_y={:.1}, distance={:.2}",
        ...
        assert!(
            max_distance <= 1.1,
            "Pixel centers should align with mathematical edge, max distance was {}",
            max_distance
        );
    }
    ...
    fn test_triangle_angles() {
        ...
        eprintln!(
            "Tip angle: {:.1} degrees (expected: ~90 degrees)",
            angle_deg
        );
        ...
        assert!(
            ...
            "Tip angle should be approximately 90 degrees, got {:.1}",
            angle_deg
        );
        ...
        eprintln!(
            "Top edge slope angle: {:.1} degrees (expected: 45)",
            angle_to_tip_top
        );

        // Since top edge slope is (x2-x1)/(y2-y1) = 14/-14 = -1, the angle is 45 degrees
        assert!(
            (angle_to_tip_top - 45.0).abs() < 1.0,
            "Top edge should be 45 degrees, got {:.1}",
            angle_to_tip_top
        );
    }
}
